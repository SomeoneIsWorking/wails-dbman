  ...
  <OverlayScrollbarsComponent class="h-full p-4">
    ...
    >
      <div
        v-for="i in 6"
        :key="i"
        class="animate-pulse bg-surface-hover h-32 rounded-lg border border-border/50"
      ></div>
    </div>
    ...
    >
      <div
        class="w-16 h-16 bg-surface-hover rounded-full flex items-center justify-center mb-6"
      >
        <Database class="w-8 h-8 text-foreground-secondary/50" />
      </div>
      <h3
        class="text-sm font-bold text-foreground uppercase tracking-widest mb-2"
      >
        No Connections
      </h3>
      <p
        class="text-xs text-foreground-secondary mb-6 max-w-[200px] mx-auto opacity-70"
      >
      ...
      >
        <Plus
          class="w-4 h-4 mr-1 group-hover:rotate-90 transition-transform duration-300"
        />
        Add Connection
      </button>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div
          class="w-8 h-8 rounded-full border border-border group-hover:border-accent/40 flex items-center justify-center transition-colors"
        >
          <Plus
            class="w-4 h-4 text-foreground-secondary group-hover:text-accent"
          />
        </div>
        <span
          class="text-sm font-bold uppercase tracking-widest text-foreground-secondary group-hover:text-accent"
          >New Connection</span
        >
      </button>
    </div>
