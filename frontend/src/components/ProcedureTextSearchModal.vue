<template>
  <div
    ...
    @click="close"
  >
    <div class="bg-surface rounded shadow-2xl w-full max-w-xl border border-border flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200" @click.stop>
      <div class="toolbar !bg-surface-hover/20 !border-b !h-12 px-4">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm font-bold uppercase tracking-widest text-foreground-secondary/50 px-1.5 py-0.5 border border-border rounded">ESC</span>
        </div>
      </div>

      <div class="flex-1 min-h-[300px] max-h-[60vh] bg-surface">
        <div v-if="isLoading" class="flex flex-col items-center justify-center py-12 gap-3 opacity-50">
          <Loader class="w-6 h-6 animate-spin text-primary" />
          <span class="text-sm font-bold uppercase tracking-widest">Searching codebases...</span>
        </div>
        ...
        >
          ...
          <span class="text-xs font-bold uppercase tracking-widest">No definitions found</span>
        </div>

        <div v-else-if="!searchQuery" class="flex flex-col items-center justify-center py-12 gap-2 opacity-30">
          <span class="text-sm font-bold uppercase tracking-widest">Type to search procedures</span>
        </div>
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center justify-between mb-0.5">
                <div class="font-bold text-xs text-foreground truncate uppercase tracking-tight">{{ result.name }}</div>
                <div class="text-sm font-bold text-foreground-secondary uppercase tracking-widest opacity-60">
                  {{ result.database }}
                </div>
              </div>
              
              <div class="text-sm text-foreground-secondary mb-2 truncate opacity-70">
      ...
      <div v-if="searchResults.length > 0" class="toolbar !bg-surface-hover/10 !border-t px-4 !h-8 text-sm font-bold uppercase tracking-widest text-foreground-secondary/70">
        <span>{{ searchResults.length }} Results Found</span>
        <div class="flex-1"></div>
        <div class="flex items-center gap-3">
          <span class="flex items-center gap-1.5"><ArrowUp class="w-3 h-3" /><ArrowDown class="w-3 h-3" /> Navigate</span>
          <span class="flex items-center gap-1.5"><ChevronRight class="w-3 h-3 rotate-90 scale-x-[-1]" /> Select</span>
        </div>
      </div>
    </div>
  </div>
...
import { Search, Settings, Loader, ArrowUp, ArrowDown, ChevronRight } from "lucide-vue-next";
