<template>
  <ScrollView class="h-full text-foreground p-4 relative">
    <StateWrapper :loading="tableState.loading" :error="tableState.error">
      <div v-if="tableState.schema" class="space-y-6">
        <!-- Columns -->
        <div>
          <h4 class="text-lg font-semibold mb-3 flex items-center gap-2 text-foreground">
          ...
          <div class="border border-border rounded-lg shadow-sm">
            <table class="min-w-full border-separate border-spacing-0">
              <thead class="bg-surface-hover">
                <tr class="uppercase">
                  <th class="top-0 z-10 px-4 py-2 border-b border-border bg-surface-hover">Name</th>
                  <th class="top-0 z-10 px-4 py-2 border-b border-border bg-surface-hover">Type</th>
                  <th class="top-0 z-10 px-4 py-2 border-b border-border bg-surface-hover">Nullable</th>
                  <th class="top-0 z-10 px-4 py-2 border-b border-border bg-surface-hover">Default</th>
                  <th class="top-0 z-10 px-4 py-2 border-b border-border bg-surface-hover">Primary</th>
                  <th class="top-0 z-10 px-4 py-2 border-b border-border bg-surface-hover">Foreign</th>
                </tr>
              </thead>
              <tbody class="bg-surface divide-y divide-border">
                <tr v-for="column in tableState.schema.columns" :key="column.name" class="hover:bg-surface-hover">
        ...
        <!-- Primary Key -->
        <div v-if="tableState.schema.primaryKey && tableState.schema.primaryKey.length > 0">
          <h4 class="text-lg font-semibold mb-3 flex items-center gap-2 text-foreground">
          ...
          <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded p-3">
            <p class="text-sm text-yellow-800 dark:text-yellow-200">
              <strong>Columns:</strong> {{ tableState.schema.primaryKey.join(', ') }}
            </p>
          </div>
        </div>

        <!-- Foreign Keys -->
        <div v-if="tableState.schema.foreignKeys && tableState.schema.foreignKeys.length > 0">
          <h4 class="text-lg font-semibold mb-3 flex items-center gap-2 text-foreground">
